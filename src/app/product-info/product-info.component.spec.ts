...
describe('ProductInfoComponent', () => {
  ...
  it('should get product from products', () => {
    ...
    expect(component['title'].setTitle).toHaveBeenCalledWith(`Product | ${data.title}`);
  })
  ...
  it('should get product id', () => {
    ...
    //For throw error
    mockActivatedRoute = {
      params: throwError(() => new Error('error')),
      queryParams: of({ sort: 'asc' }),
      snapshot: { paramMap: { get: () => '1' } },
    };
    component.getProduct();
    // expect(console.error).toThrowError('error');
  })
  ...
  it('should route to product page', () => {
    ...
    expect(component['router'].navigate).toHaveBeenCalledWith([`/products/${data.id}`])
  })
