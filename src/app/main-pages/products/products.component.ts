...
export class ProductsComponent implements OnInit, AfterViewInit, OnDestroy{
  ...
  filteringData() {
    ...
    if(this.subscription) {
      ...
      this.subscription = this.router.queryParams.pipe(
        auditTime(0)
      ).subscribe({
        next:(params) => {
          ...
          if(params['Category'] && !params['discount'] && !params['sort']) {
            ...
            this.sortFiltering(params['Category'], params['sort'], params['discount']);
          } else if (params['Category'] && params['discount'] && !params['sort']) {
  ...
  selectOption() {
    this.router.queryParams.subscribe({
      next:(params) => {
        this.selectedValues = [params['Category'], params['discount'], params['sort']];
      }
    })
  }
  ...
  discountFiltering( discount:string[]) {
    ...
    let data = from(this.productsData);
    data.pipe(
      filter((products) => products.discountPercentage > Number(discount[0]) && products.discountPercentage < Number(discount[1])),
      toArray()
    ).subscribe({
      next:(data) => {
        if (data.length == 0) {
  ...
  sortFiltering(category:string, sort:string[], discount:string[]) {
    ...
    if (category != null && discount == null) {
      ...
      data.pipe(
        map((products) => {
          return products.filter(product => product.discountPercentage > Number(discount[0]) && product.discountPercentage < Number(discount[1]));
        })
      ).subscribe(c => data = of(c));
    } else if (category != null && discount != null) {
      data.pipe(
        ...
        map((data) => {
          return data.filter(product => product.discountPercentage > Number(discount[0]) && product.discountPercentage < Number(discount[1]));
        })
      ).subscribe(c => data = of(c))
    }
    if(sort[0] === 'asc') {
  ...
  withOutSort(category:string, discount:string[]) {
    ...
    data.pipe(
      filter(products => products.category === category),
      filter((product) => {
        return product.discountPercentage > Number(discount[0]) && product.discountPercentage < Number(discount[1])
      }),
      toArray()
    ).subscribe({
      next:(data) => {
